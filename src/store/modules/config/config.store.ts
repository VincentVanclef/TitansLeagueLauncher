...
class ConfigState extends VuexModule implements IConfigState {
	...
	@Mutation
	SetSelectedRealm(realmId: number) {
		this.config!.selectedRealm = realmId;
	}
	...
	async SynchronizePatchSettings() {
	    if (!this.config || !this.config.selectedRealm) return;

		this.config.patchConfig = this.config.patchConfig.filter(x => !!x.realmId);

		const realmId = this.config.selectedRealm;
	    ...
	    if (patches.length === 0) {
			this.config.patchConfig = this.config.patchConfig.filter(x => x.realmId !== realmId);
	        await ConfigModule.SaveConfig(this.config);
	        return;
		}
		
	    for (const patch of patches) {
			const exists = this.config.patchConfig.find(x => x.patch === patch.patch && x.realmId === realmId);
	        if (!exists) {
	            this.config.patchConfig.push(patch);
	        } else {
	            exists.details = patch.details;
				exists.downloadLink = patch.downloadLink;
				exists.size = patch.size;
				exists.realmId = patch.realmId;
	        }
	    }

	    // Check for deleted stuff
		const savedPatchesForRealm = this.config.patchConfig.filter(x => x.realmId === realmId);
		for (const patch of savedPatchesForRealm) {
	        let index = patches.findIndex(x => x.patch === patch.patch && x.realmId === realmId);
	        if (index < 0) {
				index = this.config.patchConfig.findIndex(x => x.patch === patch.patch && x.realmId === realmId);
