<template>
	<b-modal id="settings-component" centered title="Settings" ok-title="Save" ok-only @ok="SaveSettings()" @show="SyncSettings()">
		<template v-slot:modal-footer="{ ok }">
			<div class="settings_footer">
				<div class="text-center button button-orange" style="height: 40px; width: 200px; font-size: 1em;" @click="SelectFolder">
					Select WoW Folder
				</div>
				<div class="text-center button button-blue" style="height: 40px; width: 100px; font-size: 1em;" @click="AppInfo()">
					App Info
				</div>
				<div class="text-center button button-green" style="height: 40px; width: 100px; font-size: 1em;" @click="ok()">
					Save
				</div>
			</div>
		</template>
		...
		<b-row v-if="settings">
			...
			<b-col cols="2" class="d-flex flex-column justify-content-end">
				<div class="settings_entry">
					<b-form-checkbox v-model="settings.autoResetRealmlist" switch size="lg"> </b-form-checkbox>
				</div>
				<div class="settings_entry">
					<b-form-checkbox v-model="settings.autoClearCache" switch size="lg"> </b-form-checkbox>
				</div>
				<div class="settings_entry">
					<b-form-checkbox v-model="settings.autoCheckForUpdates" switch size="lg"> </b-form-checkbox>
				</div>
				<div class="settings_entry">
					<b-form-checkbox v-model="settings.enableLogs" switch size="lg"> </b-form-checkbox>
				</div>
			</b-col>
		</b-row>
		<hr />
		<b-row v-if="settings">
			...
			<template v-for="config in settings.patchConfig">
				...
				<b-col cols="2" class="d-flex flex-column-reverse justify-content-end" :key="config.patch + 3">
					<div class="settings_entry">
						<b-form-checkbox v-model="config.keepUpdated" switch size="lg" :disabled="!settings.autoCheckForUpdates"> </b-form-checkbox>
					</div>
				</b-col>
			</template>
		</b-row>
